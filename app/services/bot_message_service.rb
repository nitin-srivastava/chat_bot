...
class BotMessageService
  ...
  def receive(message_hash)
    ...
    message = chat.messages.new(telegram_message_id: message_hash['message_id'], text_message: message_hash['text'])
    save_record(message)
  ...
  def find_or_create_chat(param_hash)
    ...
    chat.assign_attributes(first_name: param_hash['first_name'], last_name: param_hash['last_name'], chat_type: param_hash['type'])
    save_record(chat)
    chat
  end
